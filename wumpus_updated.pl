...
%Yunfei Added, this part was missing in the code: Start
add_wumpus_KB(yes) :-
    %agent_location(L1),
    %adjacent(L1, L2),
    %assume_wumpus(no, L2).
...
%Yunfei Added, this part was missing in the code: End
...
add_gold_KB(no) :-
%    gold_location(GL),
% 	Yunfei modified here, otherwise it assert the GL to isGold(no,GL), which is not right, and also print the wrong statement.
    agent_location(AL),
    assume_gold(no, AL).
